...
function History() {
  ...
  useEffect(() => {
    async function fetchUserDetails() {
      await fetch("/user/username/" + user.username)
        .then((res) => res.json())
        .then((data) => {
  ...
  const CellFormatter = (cell, row) => {
    return <Button onClick={handlePick}>Email</Button>;
  };

  // // This will put the donor request to complete.
  // const CellFormatter = (cell, row) => {
